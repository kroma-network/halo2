...
>(
    ...
    let instance: Vec<InstanceSingle<E::G1Affine>> = instances
        ...
        .map(|instance| -> Result<InstanceSingle<E::G1Affine>, Error> {
            ...
            E::G1::batch_normalize(&instance_commitments_projective, &mut instance_commitments);
    ...
    let advice: Vec<AdviceSingle<E::G1Affine>> = circuits
        .iter()
        .zip(instances.iter())
        .map(
            |(circuit, instances)| -> Result<AdviceSingle<E::G1Affine>, Error> {
                ...
                E::G1::batch_normalize(&advice_commitments_projective, &mut advice_commitments);
    ...
    let lookups: Vec<Vec<lookup::prover::Permuted<E>>> = instance
        .iter()
        .zip(advice.iter())
    ...
    let permutations: Vec<permutation::prover::Committed<E>> = instance
        .iter()
        .zip(advice.iter())
        .map(|(instance, advice)| {
    ...
    let lookups: Vec<Vec<lookup::prover::Committed<E>>> = lookups
        .into_iter()
    ...
    let permutations: Vec<permutation::prover::Evaluated<E>> = permutations
        .into_iter()
        .map(|permutation| -> Result<_, _> { permutation.evaluate(pk, x, transcript) })
        .collect::<Result<Vec<_>, _>>()?;

    // Evaluate the lookups, if any, at omega^i x.
    let lookups: Vec<Vec<lookup::prover::Evaluated<E>>> = lookups
        .into_iter()
        .map(|lookups| -> Result<Vec<_>, _> {
            lookups
                .into_iter()
